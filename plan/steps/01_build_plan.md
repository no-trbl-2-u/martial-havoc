...
- [x] Phase 7 — The trail-head village (brief:
      ...
      2026-09-06) — ff1fe2a
- [-] Phase 8 — The UI **(v1)** (canonical sibling: `/critique` starts
      after this phase ships; set via oversight 2026-09-05). **The
      done-condition is met**: `e2e/creation.spec.ts` creates a Master
      and wins a fight on the web export, and the layout decision is
      recorded in `design/INDEX.md`. Shipped: the layout call
      (d4ab97f, c6430ae), creation in the book's order with the eight
      printed sheets (6279620), the village (b66bd01), the campaign
      record with export and import (76c5230).
      **One item of the row's list remains: the cave's areas.** The beat
      plays areas 2-4 from the Phase 5 prototype slice; the full eight
      are shipped as data (`campaigns/the-5-treasures/areas.json`) and
      the engine's adventure loop that walks them is complete
      (`packages/engine/src/adventure`, proven to the ending on fixed
      dice). Wiring the beat onto it replaces the hand-authored menu
      with one derived from the adventure graph, which moves combat's
      foe source and rewrites the e2e — phase-sized, not a finishing
      touch. It is Phase 10's prerequisite and should be its own row or
      a deliberate 8c. **This UI is v1** — see "The UI is v1" above.
