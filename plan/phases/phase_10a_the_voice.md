...
> The one question it carried was answered by the operator on
> 2026-09-06: option 2, a named narrator. `plan/VOICE.md` names him
> and bounds him; `VISION.md` and `RE-SEED.md` carry the law. What
> remains is the wiring below.
...
**Done when:** every result kind on the beat and in combat prints the
narrator's line in his mark; the eight area lines and the five act
lines print where they belong; `result-lines.json` exists with a line
per result kind, each held to `plan/VOICE.md` by a content test; the
fidelity leg counts the lines and the count is printed by the build.

**Waits on:** nothing; the call is made and `RE-SEED.md` is written.
**Cost:** one week of wiring; the writing is 10b–10j's.
...
The operator chose **option 2** on 2026-09-06. The three, for the
record:
...
2. **A named narrator.** A storyteller persona with a consistent
   register; lines may run to two sentences. **Chosen.** He is Old
   Ping, the shuoshu of Fen Pass; `plan/VOICE.md` has his rules and
   the ten approved samples.
...
- `plan/VOICE.md` exists: third person, present tense, the Master by
  name, two sentences and forty words at most, no rule, number or
  citation in a line, nothing the dice did not roll. Ten approved
  samples.
- The mark: every narrator line prints in italic under a dashed rule
  headed OLD PING in small capitals (`ui.narrator.name`), so a reader
  tells the book from the narrator at a glance without a citation.
- The content test holds every narrator record to the guide: two
  sentences, forty words, no exclamation mark, no digit.
...
Feature: The app's voice is marked and bounded
  ...
  Scenario: a result prints the book's text and the narrator's line, told apart
    Given a fresh Master named Lin Shu on the Flat-top mountain
    When I walk to the Cave entrance on a die of 4
    Then the result slip shows "Safe exploration" as the book prints it
    And under a dashed rule headed OLD PING it shows his line for a safe entry
    And that line is in italic, names Lin Shu, and the book's text is upright
  ...
  Scenario: no authored line exceeds the guide
    ...
    Then every line is at most two sentences and forty words
    And no line carries an exclamation mark or a digit
